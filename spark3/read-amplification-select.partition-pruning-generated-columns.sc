...
This example shows how a filter based on a column that is used in a generated expression can lead to partition pruning. 
...
In this example we create a delta table with a datetime column, datetime_pp column (same value as datetime), year column also used for partitioning.

If we query the table with a filter on datetime, we will not have partition pruning. Why? There is no information on the table associating how to infer a year from the datetime.

However, if we query the table with a filter on datetime_pp we will have partition pruning. Why? The year_pp is computed as a generated column from a datetime_pp applying 
the SQL builtin function year().  This creates an association between the column datetime_pp and the column year_pp that can be exploited by the optimizer to infer 
eligible partition values.
